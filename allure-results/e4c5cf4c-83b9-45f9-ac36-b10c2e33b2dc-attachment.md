# Page snapshot

```yaml
- generic [ref=e2]:
  - region
  - generic [ref=e3]:
    - link "Skip to content" [ref=e4] [cursor=pointer]:
      - /url: "#start-of-content"
    - banner [ref=e6]
  - main [ref=e9]:
    - generic [ref=e10]:
      - generic [ref=e14]:
        - img [ref=e17]
        - heading "Sign in to GitHub" [level=1] [ref=e20]
      - generic [ref=e21]:
        - generic [ref=e24]:
          - button "Dismiss this message" [active] [ref=e25] [cursor=pointer]:
            - img [ref=e26]
          - alert [ref=e28]: This account does not support password sign-in, please try another sign-in method or account recovery.
        - generic [ref=e30]:
          - generic [ref=e31]:
            - generic [ref=e32]: Username or email address
            - textbox "Username or email address" [ref=e33]: test456
          - generic [ref=e34]:
            - generic [ref=e35]: Password
            - textbox "Password" [ref=e36]
            - link "Forgot password?" [ref=e37] [cursor=pointer]:
              - /url: /password_reset
          - button "Sign in" [ref=e39] [cursor=pointer]
        - generic [ref=e40]:
          - generic [ref=e42]: or
          - button "Continue with Google" [ref=e44] [cursor=pointer]:
            - generic [ref=e45]:
              - generic:
                - img:
                  - img
              - generic [ref=e46]: Continue with Google
          - button "Continue with Apple" [ref=e48] [cursor=pointer]:
            - generic [ref=e49]:
              - generic:
                - img:
                  - img
              - generic [ref=e50]: Continue with Apple
      - generic [ref=e51]:
        - paragraph [ref=e53]:
          - text: New to GitHub?
          - link "Create an account" [ref=e54] [cursor=pointer]:
            - /url: /signup?return_to=https%3A%2F%2Fgithub.com%2Flogin&source=login
        - paragraph [ref=e56]:
          - button "Sign in with a passkey" [ref=e57] [cursor=pointer]:
            - generic [ref=e59]: Sign in with a passkey
  - contentinfo [ref=e60]:
    - list [ref=e61]:
      - listitem [ref=e62]:
        - link "Terms" [ref=e63] [cursor=pointer]:
          - /url: https://docs.github.com/site-policy/github-terms/github-terms-of-service
      - listitem [ref=e64]:
        - link "Privacy" [ref=e65] [cursor=pointer]:
          - /url: https://docs.github.com/site-policy/privacy-policies/github-privacy-statement
      - listitem [ref=e66]:
        - link "Docs" [ref=e67] [cursor=pointer]:
          - /url: https://docs.github.com
      - listitem [ref=e68]:
        - link "Contact GitHub Support" [ref=e69] [cursor=pointer]:
          - /url: https://support.github.com
      - listitem [ref=e70]:
        - button "Manage cookies" [ref=e72] [cursor=pointer]
      - listitem [ref=e73]:
        - button "Do not share my personal information" [ref=e75] [cursor=pointer]
```